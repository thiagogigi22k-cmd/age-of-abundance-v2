...
export default function Step9Content() {
  ...
  const handleOptionClick = (index: number) => {
    ...
    setTimeout(() => {
      router.push(`/step10?name=${encodeURIComponent(name)}&balance=${encodeURIComponent(balance)}`)
    }, 1000)
  }
      ...
      <div className="relative z-10 w-full flex flex-col items-center px-4 pt-6 pb-8">
        ...
        <div className="w-full max-w-md mb-4">
          ...
          <div className="relative w-full h-[6px] bg-[#1a2744] rounded-full">
            ...
            <div className="absolute left-4 top-0 h-full bg-[#F9D423] rounded-r-full" style={{ width: "38%" }} />
          </div>
        </div>

        <div className="w-full max-w-md mb-4 px-2">
          <div className="grid grid-cols-5 gap-1.5">
            {/* Card 1 - Unlocked (I Choose My Future) */}
            ...
            {/* Card 2 - Unlocked (100x Multiplication) */}
            <div className="bg-[#0d0d1a] rounded-lg border border-[#D4AF37] p-1 flex flex-col items-center">
              <div className="w-10 h-10 rounded-lg overflow-hidden mb-1 border border-[#D4AF37]">
                <Image
        ...
        <div className="w-full max-w-md bg-[#0d0d1a]/95 rounded-xl border-2 border-[#D4AF37] p-5">
          <div className="flex justify-center mb-4">
            <span className="bg-[#2a2a3e] text-gray-300 text-sm px-4 py-1.5 rounded-full">
          ...
          {isLoading ? (
            ...
            <>
              <p className="text-white text-lg font-bold text-center mb-5 leading-relaxed">
                Do you believe God can transform your life with just one single decision from you?
              </p>

              <div className="space-y-3">
                {["Yes, absolutely", "I have doubts", "I don't know"].map((option, index) => (
                  <button
                    key={index}
