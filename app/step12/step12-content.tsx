...
export default function Step12Content() {
  ...
  const handleOptionClick = (index: number) => {
    ...
    setTimeout(() => {
      router.push(`/step13?name=${encodeURIComponent(name)}&balance=${encodeURIComponent(balance)}`)
    }, 1000)
  }
      ...
      <div className="relative z-10 w-full flex flex-col items-center px-4 pt-6 pb-8">
        ...
        <div className="w-full max-w-md mb-4">
          ...
          <div className="relative w-full h-[6px] bg-[#1a2744] rounded-full">
            ...
            <div className="absolute left-4 top-0 h-full bg-[#F9D423] rounded-r-full" style={{ width: "48%" }} />
          </div>
        </div>

        <div className="w-full max-w-md mb-4 px-2">
          <div className="grid grid-cols-5 gap-1.5">
            {/* Card 1 - I Choose My Future */}
            ...
            {[1, 2].map((i) => (
              <div
                key={i}
                className="bg-[#1a1a2e] rounded-lg border border-gray-600 p-1 flex flex-col items-center justify-center"
              >
                <div className="w-10 h-10 rounded-lg bg-[#2a2a3e] flex items-center justify-center mb-1">
                  <svg width="20" height="20" viewBox="0 0 24 24" fill="none" className="text-gray-500">
                    <rect x="5" y="11" width="14" height="10" rx="2" stroke="currentColor" strokeWidth="2" />
                    <path d="M8 11V7a4 4 0 118 0v4" stroke="currentColor" strokeWidth="2" strokeLinecap="round" />
                  </svg>
                </div>
                <span className="text-gray-500 text-[8px] font-medium">Locked</span>
              </div>
            ))}
          </div>
        </div>
          ...
          {isLoading ? (
            ...
            <>
              ...
              <p className="text-white text-lg font-bold text-center mb-5 leading-relaxed">
                If God gave you the chance to win your dream home... what would it look like?
              </p>

              {/* Answer Options - 4 options */}
              <div className="space-y-3">
                {[
                  "Large, with several rooms",
                  "A modern apartment",
                  "A simple house, but full of peace",
