...
export default function Step5Content() {
  ...
  const handleOptionClick = (index: number) => {
    ...
    setTimeout(() => {
      router.push(`/step6?name=${encodeURIComponent(name)}&balance=${encodeURIComponent(balance)}`)
    }, 1000)
  }
      ...
      <div className="relative z-10 w-full flex flex-col items-center px-4 pt-6 pb-8">
        ...
        <div className="w-full max-w-md mb-4">
          ...
          <div className="relative w-full h-[6px] bg-[#1a2744] rounded-full">
            ...
            <div className="absolute left-4 top-0 h-full bg-[#F9D423] rounded-r-full" style={{ width: "24%" }} />
          </div>
        </div>
          ...
          {isLoading ? (
            ...
            <>
              ...
              <p className="text-white text-lg font-bold text-center mb-5 leading-relaxed">
                If the life you live today was the direct result of your own choices... would you be satisfied?
              </p>

              {/* Answer Options */}
              <div className="space-y-3">
                {["No, I want to change", "More or less", "Yes, but I want more"].map((option, index) => (
                  <button
                    key={index}
