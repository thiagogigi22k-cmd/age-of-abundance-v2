...
export default function Step4Content() {
  ...
  const handleOptionClick = (index: number) => {
    ...
    setTimeout(() => {
      router.push(`/step5?name=${encodeURIComponent(name)}&balance=${encodeURIComponent(balance)}`)
    }, 1000)
  }
      ...
      <div className="relative z-10 w-full flex flex-col items-center px-4 pt-6 pb-8">
        ...
        <div className="w-full max-w-md mb-4">
          ...
          <div className="relative w-full h-[6px] bg-[#1a2744] rounded-full">
            ...
            <div className="absolute left-4 top-0 h-full bg-[#F9D423] rounded-r-full" style={{ width: "19%" }} />
          </div>
        </div>
          ...
          {isLoading ? (
            ...
            <>
              ...
              <p className="text-white text-lg font-bold text-center mb-5 leading-relaxed">
                Do you feel stuck in the same negative cycles for years?
              </p>

              {/* Answer Options */}
              <div className="space-y-3">
                {["Yes, it feels like a loop", "Sometimes I feel this", "I'm not sure"].map((option, index) => (
                  <button
                    key={index}
