...
export default function Step10Content() {
  ...
  const handleOptionClick = (index: number) => {
    ...
    setTimeout(() => {
      router.push(`/step11?name=${encodeURIComponent(name)}&balance=${encodeURIComponent(balance)}`)
    }, 1000)
  }
      ...
      <div className="relative z-10 w-full flex flex-col items-center px-4 pt-6 pb-8">
        ...
        <div className="w-full max-w-md mb-4">
          ...
          <div className="relative w-full h-[6px] bg-[#1a2744] rounded-full">
            ...
            <div className="absolute left-4 top-0 h-full bg-[#F9D423] rounded-r-full" style={{ width: "43%" }} />
          </div>
        </div>
          ...
          {isLoading ? (
            ...
            <>
              <p className="text-white text-lg font-bold text-center mb-5 leading-relaxed">
                Are you willing to make TODAY your day of faith and responsibility for your new life?
              </p>

              <div className="space-y-3">
                {["Yes, I'm ready", "I need a clearer sign", "No"].map((option, index) => (
                  <button
                    key={index}
