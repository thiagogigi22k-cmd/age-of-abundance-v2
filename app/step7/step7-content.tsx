...
export default function Step7Content() {
  ...
  const handleOptionClick = (index: number) => {
    ...
    // Show loading for 1 second then navigate to next step
    setTimeout(() => {
      router.push(`/step8?name=${encodeURIComponent(name)}&balance=${encodeURIComponent(balance)}`)
    }, 1000)
  }
      ...
      <div className="relative z-10 w-full flex flex-col items-center px-4 pt-6 pb-8">
        ...
        <div className="w-full max-w-md mb-4">
          <div className="flex justify-end mb-1">
            <span className="text-gray-400 text-sm">29%</span>
          </div>
          <div className="relative w-full h-[6px] bg-[#1a2744] rounded-full">
            ...
            <div className="absolute left-4 top-0 h-full bg-[#F9D423] rounded-r-full" style={{ width: "29%" }} />
          </div>
        </div>
        ...
        <div className="w-full max-w-md overflow-x-auto mb-4">
          <div className="flex gap-2 pb-2">
            ...
            {/* Second card - Unlocked (100x Multiplication) */}
            <div className="flex-shrink-0 w-20 bg-[#0d0d1a] rounded-lg border border-[#D4AF37] p-1.5 flex flex-col items-center">
              <div className="w-14 h-14 rounded-lg overflow-hidden mb-1 border border-[#D4AF37]">
                <Image
          ...
          {isLoading ? (
            ...
            <>
              ...
              <p className="text-white text-lg font-bold text-center mb-5 leading-relaxed">
                How many times have you asked God for a sign to change your life?
              </p>

              {/* Answer Options */}
              <div className="space-y-3">
                {["Many times", "A few times", "I don't usually ask for signs", "This is my first time"].map(
                  (option, index) => (
                    <button
                      key={index}
