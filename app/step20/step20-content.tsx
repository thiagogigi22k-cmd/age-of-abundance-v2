...
export default function Step20Content() {
  ...
  const handleOptionClick = (index: number) => {
    ...
    setTimeout(() => {
      router.push(`/step21?name=${encodeURIComponent(name)}&balance=${encodeURIComponent(balance)}`)
    }, 1000)
  }
      ...
      <div className="relative z-10 w-full flex flex-col items-center px-4 pt-6 pb-8">
        ...
        <div className="w-full max-w-md mb-4">
          ...
          <div className="relative w-full h-[6px] bg-[#1a2744] rounded-full">
            ...
            <div className="absolute left-4 top-0 h-full bg-[#F9D423] rounded-r-full" style={{ width: "67%" }} />
          </div>
        </div>

        {/* Achievement Cards - 7 cards: 5 on first row, 2 on second row */}
        <div className="w-full max-w-md mb-4 px-2">
          ...
          <div className="grid grid-cols-5 gap-1.5">
            ...
            {/* Seventh card - 100x Multiplication */}
            <div className="bg-[#0d0d1a] rounded-lg border border-[#D4AF37] p-1 flex flex-col items-center">
              <div className="w-10 h-10 rounded-lg overflow-hidden mb-1 border border-[#D4AF37]">
                <Image
          ...
          {isLoading ? (
            ...
            <>
              ...
              <p className="text-white text-lg font-bold text-center mb-5 leading-relaxed">
                If you received a large sum of money today... what would you do first?
              </p>

              {/* Answer Options - 4 options */}
              <div className="space-y-3">
                {["Pay off my debts", "Buy a house", "Fulfill my family's dream", "Invest in my future"].map(
                  (option, index) => (
                    <button
                      key={index}
