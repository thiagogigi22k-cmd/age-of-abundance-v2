...
export default function Step14Content() {
  ...
  const handleOptionClick = (index: number) => {
    ...
    setTimeout(() => {
      router.push(`/step15?name=${encodeURIComponent(name)}&balance=${encodeURIComponent(balance)}`)
    }, 1000)
  }
      ...
      <div className="relative z-10 w-full flex flex-col items-center px-4 pt-6 pb-8">
        ...
        <div className="w-full max-w-md mb-4">
          ...
          <div className="relative w-full h-[6px] bg-[#1a2744] rounded-full">
            ...
            <div className="absolute left-4 top-0 h-full bg-[#F9D423] rounded-r-full" style={{ width: "52%" }} />
          </div>
        </div>

        {/* Achievement Cards Row - 4 unlocked + 1 locked */}
        <div className="w-full max-w-md overflow-x-auto mb-4">
          <div className="flex gap-2 pb-2">
            {/* First card - Unlocked (I Choose My Future) */}
            ...
            {/* Second card - Unlocked (Faith Decision) */}
            <div className="flex-shrink-0 w-20 bg-[#0d0d1a] rounded-lg border border-[#D4AF37] p-1.5 flex flex-col items-center">
              <div className="w-14 h-14 rounded-lg overflow-hidden mb-1 border border-[#D4AF37]">
                <Image
            ...
            {/* Third card - Unlocked (Dream Home) */}
            <div className="flex-shrink-0 w-20 bg-[#0d0d1a] rounded-lg border border-[#D4AF37] p-1.5 flex flex-col items-center">
              <div className="w-14 h-14 rounded-lg overflow-hidden mb-1 border border-[#D4AF37]">
                <Image
            ...
            {/* Fourth card - Unlocked (100x Multiplication) */}
            <div className="flex-shrink-0 w-20 bg-[#0d0d1a] rounded-lg border border-[#D4AF37] p-1.5 flex flex-col items-center">
              <div className="w-14 h-14 rounded-lg overflow-hidden mb-1 border border-[#D4AF37]">
                <Image
            ...
            {/* 1 Locked card */}
            <div className="flex-shrink-0 w-20 bg-[#1a1a2e] rounded-lg border border-gray-600 p-1.5 flex flex-col items-center justify-center">
        ...
        <div className="w-full max-w-md bg-[#0d0d1a]/95 rounded-xl border-2 border-[#D4AF37] p-5">
          {/* Future Visualization Badge */}
          <div className="flex justify-center mb-4">
            <span className="bg-[#2a2a3e] text-gray-300 text-sm px-4 py-1.5 rounded-full">Future Visualization</span>
          </div>
          ...
          {isLoading ? (
            ...
            <>
              ...
              <p className="text-white text-lg font-bold text-center mb-5 leading-relaxed">
                What car do you imagine parked in front of your ideal home?
              </p>

              {/* Answer Options - 4 options */}
              <div className="space-y-3">
                {["A luxury SUV", "A comfortable sedan", "A simple but reliable car", "I don't care about cars"].map(
                  (option, index) => (
                    <button
                      key={index}
